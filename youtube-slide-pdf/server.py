...
GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz3JNIcDZuWXiEoqqw_ARRa31jcrdOWlRxjgNF3yXuMJrdXpj7xOMHREh0Spwl9nKdZ/exec"
...
def is_paid(user_id):
    ...
    if len(parts) >= 2:
        ...
        base_user = parts[0] + "_" + parts[1]

        if base_user in subs:

            return subs[base_user].get(
                "approved",
                False
            )
...
def subscribe():
    ...
    try:
        ...
        return jsonify({
            "msg": "Payment submitted. Wait for approval."
        })
